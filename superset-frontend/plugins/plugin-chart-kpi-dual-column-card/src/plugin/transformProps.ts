...
export default function transformProps(chartProps: ChartProps): KpiDualColumnCardProps {
    ...
    const valBoxColorColumn = getColumnName(getProp('val_box_color_column', 'valBoxColorColumn')) || undefined;
    const valTextColorColumn = getColumnName(getProp('val_text_color_column', 'valTextColorColumn')) || undefined;
    ...
    const valueFontSize = Number(getProp('value_font_size', 'valueFontSize', 14)) || 14;
    const keyFontWeight = Number(getProp('key_font_weight', 'keyFontWeight', 400)) || 400;
    const valueFontWeight = Number(getProp('value_font_weight', 'valueFontWeight', 600)) || 600;
    const globalKeyColor = String(getProp('global_key_color', 'globalKeyColor', '#333333') || '#333333');
    const globalValueColor = String(getProp('global_value_color', 'globalValueColor', '#000000') || '#000000');
    const textTransform = (getProp('text_transform', 'textTransform', 'none') as TextTransform);
    ...
    const containerPadding = Number(getProp('container_padding', 'containerPadding', 16)) || 16;
